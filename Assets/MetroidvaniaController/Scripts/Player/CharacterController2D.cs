...
public class CharacterController2D : MonoBehaviour
{
    ...
    public void Move(float move, bool jump, bool dash)
    {
        ...
        {
            if (playerDefense.isGuarding || playerDefense.isParrying || playerDefense.inRecovery)
            {
                move = 0f;
                jump = false;
                dash = false;
    ...
    public void ApplyDamage(float damage, Vector3 position)
    {
        if (!invincible)
        {
            ...
            if (playerDefense != null)
            {
                if (playerDefense.isParrying)
                {
                    playerDefense.OnParrySuccess();
                    StartCoroutine(MakeInvincible(0.5f));
                    return;
                }
                else if (playerDefense.inRecovery)
                {
                    finalDamage *= playerDefense.failPenalty;
                }
                else if (playerDefense.isGuarding)
                {
                    finalDamage *= playerDefense.damageReduction;
                    hitShield = true;
                }
            }
            ...
            float knockbackScale = hitShield ? 0.5f : 1.0f;
            m_Rigidbody2D.AddForce(damageDir * 10 * knockbackScale);
